...
@Slf4j
class GuidanceProvider extends AbstractGuidanceRecommendationProvider implements GuidanceRecommendationProvider.ExecutableFacet{
	String code = "morpheus-guidance-agent-upgrade"
	String name = "Agent Upgrade"

	MorpheusContext morpheusContext
	Plugin plugin

	GuidanceProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
	...
	void calculateRecommendations() {
		log.info("Running Guidance Checks for Agent Upgrades")
	...
	HTMLResponse renderTemplate(AccountDiscovery accountDiscovery) {
		...
		getRenderer().renderTemplate("hbs/agentUpgrade", model)
	}

	@Override
	Icon getIcon() {
		return new Icon(path: "upgrade.png", darkPath: "upgrade.png")
	}
	...
	ServiceResponse<AccountDiscovery> execute(AccountDiscovery accountDiscovery) {
		log.info("Executing Agent Upgrade for ${accountDiscovery.refName ?: accountDiscovery.refId}")
		morpheusContext.queueUpgradeServerAgent(accountDiscovery.refId)
